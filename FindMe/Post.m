...
-(MatchParser*)getMatch
{
    if (_match&&[_match isKindOfClass:[MatchParser class]]) {
        ...
        MatchParser *parser=[[self.class shareCacheForPost] objectForKey:key];
        if (parser) {
            _match=parser;
            parser.data=self;
            return parser;
        }else{
            parser=[self createMatch:263];
            if (parser) {
                [[self.class shareCacheForPost]  setObject:parser forKey:key];
            }
            return parser;
        }
    }
    ...
    if (_match&&[_match isKindOfClass:[MatchParser class]]) {
        ...
        if (parser) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                MatchParser*parser=[self createMatch:263];
                if (parser) {
                    _match=parser;
                    [[self.class shareCacheForPost]  setObject:parser forKey:key];
                    if (complete) {
