...
- (void)followScrollView:(UIView*)scrollableView withDelay:(float)delay
{
    ...
    if (IOS7_OR_LATER) {
        if (!self.navigationController.navigationBar.barTintColor) {
            MJLog(@"[%s]: %@", __PRETTY_FUNCTION__, @"[AMScrollingNavbarViewController] Warning: no bar tint color set");
        }
	...
	if ([self.navigationController.navigationBar isTranslucent]) {
		MJLog(@"[%s]: %@", __PRETTY_FUNCTION__, @"[AMScrollingNavbarViewController] Warning: the navigation bar should not be translucent");
	}
...
- (void)scrollWithDelta:(CGFloat)delta
{
	...
	if (delta < 0) {
        ...
        // Prevents the navbar from moving during the 'rubberband' scroll
        if ([self contentoffset].y + self.scrollableView.frame.size.height > [self contentSize].height+64) {
            return;
        }
		if (self.collapsed) {
            self.collapsed = NO;
